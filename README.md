...
```bash
apple-checker$ sam logs -n StockCheckerFunction --stack-name apple-checker --tail
```
...
```bash
apple-checker$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name apple-checker
```
